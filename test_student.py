...
class TestStudent(unittest.TestCase):
    ...
    * Call a method named apply_extension that will take a number of days
    as an argument on the student instance to update the end_date
    * Assert whether the instance's end_date equals the old end date plus
     the days argument that was passed in using timedelta
    * Run the tests to confirm that the new method is failing
    In the Student calls, create a new moethod called apply_extension that
     has a parameter called days
    Use the timedelta method from datetime to update the end_date property
    * Run the tests to confrim they are working
    """
